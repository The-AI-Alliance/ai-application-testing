...
class TestBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.shell = shell
        self.confident_results_count: int = 0
        self.low_confidence_results: [LowConfidenceResult] = []

    def tearDown(self):
        if self.low_confidence_results:
            print(f"{__file__}: {len(self.low_confidence_results)}/{self.confident_results_count} results:")
    ...
    def try_query(self, 
        ...
        if confidence >= confidence_threshold and rating >= rating_threshold:
            ...
            for key in place_holders:
                expected = exp_place_holders.get(key)
                actual   = reply.get(key)
                self.assertEqual(expected, actual, f"for key = {key}: {err_msg}")
            self.confident_results_count += 1
        else:
    ...
    def try_queries(self, file_name: str, place_holders: dict[str,str], sample_rate: float = 1.0, rating_threshold: int = 4, confidence_threshold: float = 0.6):
