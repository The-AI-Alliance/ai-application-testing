...
def replacement_keys(min_key_size: int = 1, max_key_size: int = 5, min_size: int = 0, max_size: int = 10):
    """
    Hypothesis strategy for generating a list of strings that do not contains
    `{{...}}`.
    """
    return st.lists(st.text(
        alphabet=st.characters(codec="ascii"), min_size=min_key_size, max_size=max_key_size
    ).map(lambda s: re.sub(r"[{}]+", "_", s)), min_size=min_size, max_size=max_size)
