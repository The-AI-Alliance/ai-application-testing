...
class TestAppointmentManager(unittest.TestCase):
    ...
    def _results_list_expected(
        self, expected: Sequence[Mapping[str, Any]], actual: Sequence[Mapping[str, Any]]
    ):
    ...
    def test_create_appointment_fails_if_datetime_in_the_past(
        self, appointment_dict: dict[str, Any]
    ):
        """
    ...
    @given(appointment_dicts())
    def test_create_appointment_duplicate_time_fails(
        self, appointment_dict: dict[str, Any]
    ):
    ...
    @given(appointment_dicts())
    def test_cancel_appointment_succeeds_if_it_exists(
        self, appointment_dict: dict[str, Any]
    ):
    ...
    def test_get_appointments_with_patient_name_filter_returns_appointments_for_that_patient(
        ...
        expected_list = list(
            filter(
                lambda a: a["patient_name"] == patient_name,
                self.tool.get_appointments(),
            )
        )
    ...
    @given(appointment_dicts_lists())
    def test_get_appointments_count_returns_the_number_of_appointments(
        self, appointment_dicts_lists: list[dict[str, Any]]
    ):
        self.tool.clear()
        ids = []
        for d in appointment_dicts_lists:
            ...
            ids.append(appointment["id"])
        self.assertEqual(
            len(appointment_dicts_lists), self.tool.get_appointments_count()
        )

    @given(appointment_dicts())
    def test_get_appointment_returns_nonempty_dict_if_it_exists(
        self, appointment_dict: dict[str, Any]
    ):
        self.tool.clear()
    ...
    @given(appointment_dicts())
    def test_get_appointment_returns_empty_dict_if_key_does_not_exist(
        self, appointment_dict: dict[str, Any]
    ):
        self.tool.clear()
        appointment2 = self._check_success(appointment_dict)
    ...
    @given(appointment_dicts())
    def test_get_appointment_id_for_name_and_date_time_returns_nonempty_if_match_exists(
        self, appointment_dict: dict[str, Any]
    ):
        self.tool.clear()
        appointment2 = self._check_success(appointment_dict)
    ...
    def test_get_appointment_id_for_name_and_date_time_returns_empty_if_match_does_not_exist(
        ...
        id = self.tool.get_appointment_id_for_name_and_date_time(name + "bad", dt)
        self.assertEqual("", id, f"{name+'bad'}, {appointment_dict}")

        bad_dt = dt + timedelta(seconds=10)
        id = self.tool.get_appointment_id_for_name_and_date_time(name, bad_dt)
    ...
    @given(appointment_dicts_lists().filter(lambda lst: len(lst) > 0))
    def test_to_json_from_json_are_reversible(
        self, appointment_dicts_lists: list[dict[str, Any]]
    ):
        self.tool.clear()
        [self._check_success(d) for d in appointment_dicts_lists]
        am_json = self.tool.to_json()
        am2 = AppointmentManager.from_json(am_json)
        self.assertIsNotNone(am2, am_json)
        self.assertEqual(
            self.tool.storage.storage_path,
            am2.storage.storage_path,
            f"{am_json} -> {am2}",
        )
