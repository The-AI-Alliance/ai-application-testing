...
class UnitBenchmarkDataParent(ABC):
    ...
    def __init__(
        ...
        if not use_cases:
            ...
            errors = []
            for uc in use_cases:
                if uc not in all_ucs:
                    errors.append(uc)
                else:
                    self.use_cases[uc] = all_ucs[uc]
            if errors:
                raise ValueError(
                    f"One or more specified use cases [{', '.join(errors)}] not in known list: {list(all_ucs.keys())}"
                )
        ...
        try:
            with open(data_file, "r", encoding="utf-8") as f:
                ...
                if len(unexpected_lines) > 0:
                    self.logger.warning(
                        f"{len(unexpected_lines)} lines in {data_file} do not have expected label {expected_label}!"
                    )
                    self.logger.warning(
                        "The labels may be correct for the actual question generated, so please check them. Here they are:"
                    )
