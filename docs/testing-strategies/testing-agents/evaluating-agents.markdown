...
> **Highlights:**
>
> 1. Agent testing starts with the same tools and techniques (e.g., benchmarks) that have been used for models themselves, but agents enable more advanced workflows, such as sessions, which require additional tools beyond what we have used so far for handling benchmarks based on Q&A data pairs.
> 1. The diversity of agent behaviors has led to an explosion of general-purpose and domain-specific agent tool kits and testing benchmarks. This trend is driving interest in standardizing how benchmarks are written and executed.
...
* **Examples:** The post discusses evaluation examples for three use cases.
    ...
    * **Research agents:** Agents that gather, synthesize, and analyze information, then prepare a report for human consumption (e.g., the AI Alliance's [Deep Research Agent for Applications](https://the-ai-alliance.github.io/deep-research-agent-for-applications/){:target="_blank"} mentioned elsewhere). There are partially subjective qualities, like &ldquo;comprehensiveness&rdquo;, &ldquo;well-sourced&rdquo;, and &ldquo;correct&rdquo;, as well as a mix of more objective outcomes, like were particular, essential facts retrieved? Frequent evaluation by human experts is necessary to ensure these agents work as desired.
...
A complementary project is [Every Eval Ever](https://github.com/evaleval/every_eval_ever){:target="eee-gh"} (see [references]({{site.referencesurl}}#evaleval-coalition)), which attempts to standardize how evaluation results are reported. This project also wants to catalog all AI evaluations that follow its standards.
...
* [Agent Beats](https://agentbeats.dev/){:target="agent-beats"} is registry for agents, benchmarks, and a running competition organized by the University of California at Berkeley's [Center for Responsible, Decentralized Intelligence](https://rdi.berkeley.edu/){:target="berkeley-rdi"} (RDI) and their [Agentic AI MOOC](https://agenticai-learning.org/f25){:target="mooc"}.
...
* [Agents' Last Exam](https://agents-last-exam.org/) (ALE) is a new project that attempts to evaluate agents doing real work. Lead by Berkeley's RDI, it is a collaboration with over 300 global experts across 55 industries. ALE is a benchmark designed to test that claims about agent performance on real digital labor-market work. Their findings at the time of this writing is that, &ldquo;The age of useful agents is here. The age of truly job-ready agents is not.&rdquo;
