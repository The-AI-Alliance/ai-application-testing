...
However, the idea of _what_ you should tune is also changing. [Laurie Voss posted](https://www.linkedin.com/pulse/end-fine-tuning-laurie-voss-0xpac/){:target="lv0"} recently that use of fine tuning of models never became widespread outside &ldquo;elite&rdquo; users. Instead, the emerging alternative is continual learning for the _harnesses_ (code) that _surround_ the models. [LangChain's](http://www.langchain.com){:target="lc"} Harrison Chase [wrote about the details](https://www.langchain.com/blog/continual-learning-for-ai-agents){:target="lc-claa"}, comparing the effort and benefits of tuning models, harnesses, and also the context used in agents.
